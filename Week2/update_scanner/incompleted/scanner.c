...
extern int lineNo; // Chi so dong hien tai
extern int colNo; // Chi so cot hien tai
...
Token *readNumber(void)
{
  ...
  while (charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_PERIOD)
  {
    ...
    else if (check_period > 1)
    {
      error(ERR_INVALIDFLOAT, token->lineNo, token->colNo);
    }
    else if (count_digit > MAX_NUMBER_LEN)
    {
      error(ERR_NUMBERTOOLONG, token->lineNo, token->colNo);
    }
    
    token->string[count_digit] = currentChar;
    count_digit++;
    readChar();
  }
  token->string[count_digit] = '\0';
  token->value = atoi(token->string);
  return token;
}
// Doc mot token tinh tu vi tri hien tai
Token *getToken(void)
{
  Token *token;
  int ln, cn;
